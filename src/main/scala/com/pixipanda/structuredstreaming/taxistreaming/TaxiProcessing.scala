...
import com.pixipanda.structuredstreaming.taxistreaming.utils.PointInPoly
...
object TaxiProcessing {
  ...
  def appendStartEndNeighbourhoods(df: DataFrame, sparks: SparkSession): DataFrame = {
    ...
    val lookupTable = sparks.read.json("src/main/resources/nbhd.jsonl").select("name", "coord")
